...
async function generateMediaKit(id=null){
  ...
  const dashboardState = JSON.parse(localStorage.getItem(Shared.DASHBOARD_STORAGE_KEY) || '{}');
  const savedKits = dashboardState.kits || [];
  dashboardState.kits = [kit, ...savedKits.filter(k => k.id !== kit.id)];
  localStorage.setItem(Shared.DASHBOARD_STORAGE_KEY, JSON.stringify(dashboardState));
