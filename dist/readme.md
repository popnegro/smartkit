...
Aplicación estática para publicar un catálogo de pantallas DOOH, gestionarlas desde un dashboard local y generar media kits.
...
El proyecto está configurado para un despliegue continuo y automatizado en **Vercel**.

Cada vez que se realiza un `push` a la rama `main`, Vercel se encarga de:
1.  Instalar las dependencias de build con `npm install`.
2.  Ejecutar el script de construcción (`./build.sh`) para generar el sitio estático optimizado en la carpeta `dist/`.
3.  Desplegar el contenido de `dist/` a producción.
4.  Desplegar las funciones sin servidor que se encuentren en la carpeta `/api`.

No es necesario ejecutar el build localmente ni hacer commit de la carpeta `dist/`. El archivo `vercel.json` en la raíz del proyecto contiene la configuración de build y los redireccionamientos necesarios para Vercel.
